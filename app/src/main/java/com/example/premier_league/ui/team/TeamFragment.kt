...
import com.example.premier_league.data.model.Coache
import com.example.premier_league.data.model.Player
...
class TeamFragment : BaseFragment(), TeamContact.View, View.OnClickListener {
    ...
    override fun showCoach(coaches: List<Coache>) {
        if (coaches.isEmpty()) {
            view?.context?.showToast(R.string.err_no_coach)
            return
        }
        val coachOne = coaches.first()
